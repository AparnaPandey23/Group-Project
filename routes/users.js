      ...
      try {
        User.findOne({ _id: userid }, function(err, user) {
          ...
          if (user) res.json({ user_name: user.user_name });
            ...
            res.status(401).send({ status: "error", body: "User not found" });
        });
      } catch (err) {
...
router.post("/upDateName", function(req, res, next) {
  ...
  var id = req.body.id;
  User.updateOne({ _id: id }, { $set: { full_Name: name } }, function(
    err,
    user
  ) {
    if (err) res.send(err);
...
router.post("/upDateMobile", function(req, res, next) {
  ...
  var id = req.body.id;
  User.updateOne({ _id: id }, { $set: { mobile: changeOB } }, function(
    err,
    user
  ) {
    if (err) res.send(err);
...
router.post("/upDateLandLine", function(req, res, next) {
  ...
  User.updateOne({ _id: id }, { $set: { land_Line: changeOB } }, function(
    err,
    user
  ) {
    if (err) res.send(err);
...
router.post("/upDateEmail", function(req, res, next) {
  ...
  var id = req.body.id;
  User.updateOne({ _id: id }, { $set: { email: changeOB } }, function(
    err,
    user
  ) {
    if (err) res.send(err);
...
router.post("/upDateHomeadd", function(req, res, next) {
  ...
  var id = req.body.id;
  User.insertOne({ _id: id }, { $set: { home_add: changeOB } }, function(
    err,
    user
  ) {
    if (err) res.send(err);
...
router.post("/upDateworkAdd", function(req, res, next) {
  ...
  var id = req.body.id;
  User.updateOne({ _id: id }, { $set: { work_add: changeOB } }, function(
    err,
    user
  ) {
    if (err) res.send(err);
